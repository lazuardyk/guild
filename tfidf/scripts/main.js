...
function showTfidfTable(str1, str2, str3, str4) {
  ...
  var tokens = [...new Set([...tokensAll].filter((x) => !stopwordsSet.has(x)))];

  parse = (t) =>
    tokens.map((w, i) => t.reduce((a, b) => (b === w ? ++a : a), 0));
  ...
  for (var i = 0; i < tokens.length; i++) {
    var tableRow =
      ...
      `</td>
                  <td>` +
      Math.round(idfNorm[i] * 1000) / 1000 +
      `</td>
                  <td>` +
      Math.round(tfidf1[i] * 1000) / 1000 +
      `</td>
                  <td>` +
      Math.round(tfidf2[i] * 1000) / 1000 +
      `</td>
                  <td>` +
      Math.round(tfidf3[i] * 1000) / 1000 +
      `</td>
                  <td>` +
      Math.round(tfidf4[i] * 1000) / 1000 +
      `</td>
            </tr>`;
